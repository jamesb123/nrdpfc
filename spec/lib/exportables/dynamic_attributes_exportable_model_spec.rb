...
describe Exportables::DynamicAttributesExportableModel, "in Organism" do
  ...
  it "should generate sql select elements for dynamic columns (integer_value), with the table alias" do
    ...
    query_piece.join.should == [
      "LEFT JOIN dynamic_attribute_values as organism_dynamic_attribute_nea ON (organism_dynamic_attribute_nea.owner_type = 'Organism' and organism_dynamic_attribute_nea.owner_id = organisms.id and dynamic_attribute_id = #{dynamic_attributes(:nea).id})"
    ]
  end
  ...
  it "should generate sql select elements for dynamic columns (text_value), with the table alias" do
    ...
    query_piece.join.should == [
      "LEFT JOIN dynamic_attribute_values as organism_dynamic_attribute_notes ON (organism_dynamic_attribute_notes.owner_type = 'Organism' and organism_dynamic_attribute_notes.owner_id = organisms.id and dynamic_attribute_id = #{dynamic_attributes(:notes).id})" 
    ]
  end
